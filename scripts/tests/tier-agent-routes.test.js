...
function setup() {
  ...
  registerTierAgentRoutes(app, {
    config: { PORT: 3000, TUNNEL_DOMAIN: '', AGENT_REPORT_MAX_PER_MINUTE: 12, AGENT_REPORT_MAX_CONCURRENT: 2, NODE_TEMP_WARN_C: 80, NODE_TEMP_CRITICAL_C: 90 },
    auth: (req, res, next) => req.get('authorization') === 'Bearer valid' ? next() : res.status(401).json({ error: 'Unauthorized' }),
