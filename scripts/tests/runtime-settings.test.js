...
test('runtime-settings: describe reports compose value, override state, and effective value', () => {
  ...
  assert.deepStrictEqual(groups.map(g => g.id), ['request_progress', 'capacity', 'pending_approvals', 'stuck', 'escalation', 'season_pack', 'episode_recovery', 'premiumize', 'release_group_blocklist']);
