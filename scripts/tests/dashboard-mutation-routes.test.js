...
function fixture(overrides = {}) {
  ...
  const deps = {
    ...
    previewAutomation: async name => [{ name }],
    rateLimit,
    ...
    runEpisodeRecoverySweep: async () => ({ acted: 0 }),
    runEscalation: async () => ({ ok: true, detail: 'Escalated.' }),
    runGuardedSweep: async (name, sweep) => {
      state.sweeps.push(name);
      return { ok: true, result: await sweep() };
    },
    ...
    settingsStore: {},
    sonarrSeriesAliases: () => [],
    sweepEscalations: async () => ({ acted: 1 }),
    sweepSeasonPacks: async () => ({ searched: 1 }),
    sweepStuckDownloads: async () => ({ alerted: 2 }),
...
test('preview and run-now routes preserve success, busy, and unknown responses', async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(state.sweeps, ['stuck']);
  } finally {
    await close(server);
  }

  const busyFixture = fixture({ runGuardedSweep: async () => ({ ok: false, result: { busy: true } }) });
