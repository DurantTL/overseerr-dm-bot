...
test('tier: primitives (recencyDecay, titleKey, toRelPath)', () => {
...
test('tier: computeUniversalCore (top-K by summed plays across nodes)', () => {
...
test('tier: budget fill correctness (first run: floor + ranked admits until budget)', () => {
...
test('tier: headroom (budget = usable * (1 - headroom%))', () => {
...
test('tier: graceful degradation (budget >= library => empty drop)', () => {
...
test('tier: full master is never pruned, regardless of budget', () => {
  const inv = [title('A', 10, 'm/A'), title('B', 10, 'm/B')];
...
test('tier: smaller-first tie-break on equal value', () => {
...
test('tier: incremental eviction (evict only enough to admit, coldest first)', () => {
...
test('tier: equal-value eviction takes the larger victim first, so fewer titles churn per apply', () => {
...
test('tier: over-budget trim sheds the minimum (smaller victim first), does not underfill', () => {
...
test('tier: a missing added date is "unknown", never fresh (no bogus force-keep)', () => {
...
test('tier: warm titles are never evicted', () => {
...
test('tier: fresh titles are never evicted (grace before any watch history)', () => {
...
test('tier: admission gate rejects a candidate colder than every victim', () => {
...
test('tier: floor (Tier 0) is never evictable, even when it busts the budget', () => {
...
test('tier: sticky hysteresis doubles the warm window on old-drive nodes', () => {
...
test('tier: planTier access modes — open node ignores membership and never pins requests', () => {
...
test('tier: planTier access modes — restricted node pins a member request within its grace window', () => {
...
test('tier: planTier access modes — restricted prefers own history over the universal core under pressure', () => {
...
test('tier: planTier atime demand source (LRU by last read), floor still protected', () => {
...
test('tier: computeNodeValues rolls atime file reads up to the owning title folder', () => {
...
test('tier: master coverage invariant — no enabled full node at all means nothing may be dropped', () => {
...
test('tier: disabled node history never feeds the universal core', () => {
...
test('tier: manifest rendering (renderSyncthingStignore / renderRclone)', () => {
...
test('tier: plan hash is stable across ordering and timestamps, sensitive to outcome', () => {
...
test('tier: atime maintenance-window mask (Plex nightly analysis must not count as watches)', () => {
...
test('tier: resolveTitleFolder — longest-prefix match, folder-relative path, fallback', () => {
...
test('tier: R2.1 multi-folder — same title in two folders stays disambiguated by folder id', () => {
...
test('tier: R2.1 single-folder fallback unchanged — legacy node yields one default folder', () => {
...
test('tier: R2.2 plex demand — real playback outranks an older atime-only read', () => {
...
test('tier: R2.2 plex demand — missing Plex record falls back to atime; neither is coldest', () => {
...
test('tier: R2.2 plex demand — unreachable local Plex falls back to atime for the whole node', () => {
...
test('tier: planner health warnings — keep-set forced over budget warns, does not fail', () => {
  const inv = [title('tmdb:1', 10, 'movies/M1'), title('tmdb:2', 10, 'movies/M2')];
...
test('tier: planner health warnings — dead demand signal (atime node, no report) warns', () => {
  const inv = [title('tmdb:1', 10, 'movies/M1'), title('tmdb:2', 10, 'movies/M2')];
...
test('tier: fetchTierInventory — a date-less Radarr/Sonarr item loads addedAt=null, not "now"', async () => {
...
test('tier: §1.4 anti-churn — a marginal candidate cannot displace kept titles', () => {
...
test('tier: §1.4 anti-churn — do not shred several titles for one that only wins in aggregate', () => {
...
test('tier: §1.4 anti-churn — falls back to a lower-churn victim set instead of rejecting outright', () => {
...
test('tier: §1.4 anti-churn — size-scaled transfer penalty blocks a huge marginal download', () => {
  // Candidate at 0.10 vs a zero-value victim: the absolute rule alone would pass (0.10 >= 0.05),
  // but a 2 TB transfer penalty (0.05/TB x 2 = 0.10) drags the net gain to 0.00, below 0.05.
  const TB = 1024 ** 4;
...
test('tier: §1.5 physicalTitleIds — presence from the agent file report (folder dir-walk)', () => {
  const inv = [title('tmdb:1', 10, 'movies/M1'), title('tmdb:2', 10, 'movies/M2'), title('tmdb:3', 10, 'movies/M3')]
    .map(t => ({ ...t, folderId: '' }));
  const files = [
...
test('tier: §1.5 assessApplyImpact — real removals, new downloads, and cap flags', () => {
...
test('tier: §1.5 tierApplyConfirmCode — deterministic per (node, planHash), moves when the plan moves', () => {
...
test('tier: §1.6 mediaIdFromGuid — modern + legacy Plex guid forms, type-gated', () => {
...
test('tier: §1.6 indexHistory — id-keyed and title-keyed lookups, conservative roll-up', () => {
  const hist = [
...
test('tier: §1.6 computeNodeValues joins by GUID first, title only as fallback', () => {
...
test('tier: §1.3 physicalTitleBytes + computeTierActionPreview', () => {
  const manifest = {
    keep: [
...
test('tier: a failing media source is reported, because a partial inventory blanks the .stignore', async () => {
  // This is the failure the apply block exists for: an arr that doesn't answer contributes no
  // titles, those titles land in neither keep nor drop, and the folder's .stignore renders EMPTY
  // — so the node re-downloads everything the previous plan was holding back. The apply-impact
  // caps can't catch it (the titles aren't in the keep set either), so fetchTierInventory has to
  // name the failure and the caller has to refuse. Both halves are asserted here.
  const axios = require('axios');
  const realGet = axios.get;
  axios.get = async (url) => {
  ...
  const { planTier: planTierLocal, renderFolderStignore: renderFolderStignoreLocal } = require('../../src/tier');
  ...
  const mkPartialTitle = (id, type, folderId, relPath, sizeGb) => ({
    mediaId: id, title: id, mediaType: type, sizeBytes: sizeGb * GB, addedAt: null,
    path: `/src/${relPath}`, relPath, folderId,
  });
  const complete = [
    ...Array.from({ length: 10 }, (_, i) => mkPartialTitle(`tmdb:${i}`, 'movie', 'mov-1', `Movies/M${i}/M${i}.mkv`, 50)),
    ...Array.from({ length: 2 }, (_, i) => mkPartialTitle(`tvdb:${i}`, 'tv', 'tv-1', `TV/S${i}`, 20)),
  ];
  const ignoreLines = inventory => {
    const m = planTierLocal({ nodes: [master, edgeNode], inventory, atimeReports: { edge: [] } }).manifests.edge;
    const folder = m.folders.find(f => f.folder_id === 'mov-1');
    return renderFolderStignoreLocal(folder, m).trim().split('\n').filter(l => l && !l.startsWith('//')).length;
  };
