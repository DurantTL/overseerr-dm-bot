...
- `BYPARR_URL` — adds Byparr to health checks and `/indexers`; general status uses the fast
  ...
  deep check
- `HUNTARR_URL`, `RECYCLARR_URL`, `CLEANUPARR_URL`, `SYNCTHING_URL` — base URLs that add
  Huntarr, Recyclarr, Cleanuparr, and Syncthing to the dashboard **Director** tab's fleet
  health board (any HTTP response under 500 counts as up; unset = "not configured")
...
- `PREMIUMIZE_API_KEY` — enables `/debrid status` (fair-use %, cloud storage, active/failed transfers, plus **Clear Stuck/0%** and **Clear Finished** buttons) and the **stuck-transfer watchdog**: every `PREMIUMIZE_CHECK_MINUTES` (default `15`, `0` disables) transfers that are errored or whose progress hasn't moved for `PREMIUMIZE_STUCK_AFTER_MINUTES` (default `45` — catches "0% forever") are checked. A transfer still at (effectively) 0% with no cached source (`PREMIUMIZE_AUTO_CLEAR_MAX_PROGRESS`, default `1`%) is deleted automatically (`PREMIUMIZE_AUTO_CLEAR_DEAD`, default on) instead of sitting in the queue re-alerting forever; anything else stuck gets a batched alert (one embed for the whole sweep, not one per transfer) with **Retry / Clear Transfer / Ignore** — a single stuck transfer keeps its own buttons, a batch gets **Clear Stuck/0%** / **Clear Finished** — at most once per transfer per `PREMIUMIZE_ALERT_COOLDOWN_HOURS` (default `6`); also feeds the
  dashboard **Director** tab's Premiumize liveness check
- `PREMIUMIZE_STAGING_PATH`, `PREMIUMIZE_IMPORT_PATH` (defaults to `PREMIUMIZE_STAGING_PATH`) — enable `/debrid import` and `/debrid staging`, the Premiumize-side equivalents of `/rtorrent import`/`/rtorrent staging` below. Point them at the folder a Premiumize downloader (e.g. [Premiumizearr](https://github.com/ensingerphilipp/Premiumizearr-Nova)'s `DownloadsDirectory`) syncs completed cloud transfers into — the same folder Sonarr/Radarr's Blackhole download client watches. For a multi-season pack dropped straight into Premiumize's "My Files" that the arr's watch-folder scan didn't pick up: `/debrid staging` shows why (per-folder match/rejection summary), `/debrid import` re-triggers the scan. Both stay unavailable until at least `PREMIUMIZE_IMPORT_PATH` is set. Both import commands (and `/rtorrent import`) preview the scan with the arr's own manual-import API first when `mode:move` (the default): if the folder has any file that won't match — the exact way a multi-season pack loses its unmatched seasons, since the arr clears the *whole* folder once it considers the download handled, not just the files it imported — the command stops short and asks **Import Anyway / Cancel** instead of triggering the scan blind. `mode:copy` skips the check since nothing is deleted either way.
...
- `RTORRENT_ADOPT_ENABLED` (default `false`), `RTORRENT_ADOPT_CHECK_MINUTES` (default `5`), `RTORRENT_ADOPT_LABELS` (default `sonarr,radarr,radarr-4k`), `RTORRENT_ADOPT_AUTO` (default `false`), `RTORRENT_REMOTE_ROOT` (unset) — **adoption** of torrents that already exist in the seedbox rTorrent (`/rtorrent`); see "Adopting existing torrents" in [docs/avistaz-pipeline.md](docs/avistaz-pipeline.md). Manual `/rtorrent adopt` works whenever the direct-grab transfer pieces are configured; `RTORRENT_ADOPT_ENABLED` gates only the discovery sweep.
- `RTORRENT_RATIO_CLEANUP_ENABLED` (default `false`), `RTORRENT_RATIO_CLEANUP_CHECK_MINUTES` (default `60`), `RTORRENT_RATIO_MIN_PERMILLE` (default `500`), `RTORRENT_RATIO_STALL_DAYS` (default `7`), `RTORRENT_RATIO_FORCE_PERMILLE` (default `2000`) — **ratio-based cleanup**: removes a finished torrent from rTorrent (`d.erase`, not the seedbox data — plain rTorrent XML-RPC has no call to delete files) once its ratio has sat unchanged at/above `RTORRENT_RATIO_MIN_PERMILLE` for `RTORRENT_RATIO_STALL_DAYS`, or once it reaches `RTORRENT_RATIO_FORCE_PERMILLE` at all. Thresholds are rTorrent's own ratio unit (ratio × 1000 — `500` = 0.5, `2000` = 2.0). Off by default; all five are dashboard-tunable. Every run that removes anything posts a summary to the cleanup channel splitting removals into **safe to delete on the seedbox** (this bot's own `grab_jobs` record shows the release reached its `verified` state — confirmed imported into the media library, with the rclone remote path) and **leave alone for now** (no confirmed import — an adopted/foreign torrent, or one still mid-pipeline) — informational only, nothing on the seedbox is ever deleted automatically.
...
- Passkeys use `DASHBOARD_PUBLIC_URL` (defaulting to `https://TUNNEL_DOMAIN` if unset) as their exact relying-party ID/accepted origin. Enrol them through that public URL. Credentials enrolled on `localhost`, another hostname, or an HTTP origin will not work. Opening the dashboard at any other valid HTTPS hostname than the configured one (an IP, a different Cloudflare hostname, a reverse-proxy alias) makes the browser refuse passkey enrollment/sign-in — the dashboard detects that mismatch itself before calling the browser and links you to the correct URL. Set `DASHBOARD_PUBLIC_URL` explicitly only if the dashboard has its own dedicated public hostname distinct from downloads/webhooks.
