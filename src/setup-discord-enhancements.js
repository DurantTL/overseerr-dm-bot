...
// the /send-setup admin command, Plex-invite verification, one-time Tailscale key
// provisioning, and admin tailnet help.
//
// Explicit interaction registration (no Discord prototype patching): index.js instantiates
// createSetupEnhancementsFeature with its dependencies and calls handleInteraction directly
// from its own interactionCreate listener, between the device-state and extension features,
// preserving the first-refusal order the old emit wrappers had by install order. The
// /send-setup slash command is registered through index.js's slashCommands array.
...
const { LABELS } = require('./setup-device-state');
...
function createSetupEnhancementsFeature({
  getUserByDiscordId,
  audit,
  db,
  getPlexToken,
  ...
  if (!getUserByDiscordId || !audit || !db || !getPlexToken || !fetchPlexFriends || !inviteUserToPlex
    || !createViewerAuthKey || !tailscaleApiConfigured || !provisionConfig
    || !deviceConfirmations || !anyDeviceConfirmed || !log) {
    throw new TypeError('Setup enhancements dependencies are required');
  }
...
module.exports = {
  createSetupEnhancementsFeature,
  owns,
  keyUsageCopy,
};
