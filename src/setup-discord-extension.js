...
// Discord integration for the guided setup center. Explicit interaction registration with
// dependency injection (no Discord prototype patching): index.js instantiates
// createSetupExtensionFeature and calls handleInteraction directly from its own
// interactionCreate listener. Keeping it isolated avoids another large block of
// onboarding/button code in index.js while the main composition root is being broken
// into services.
...
// Explicit button/modal/command handler registration with dependency injection, matching the
// pattern src/media-panel.js established: no Discord prototype patching here at all — index.js
// calls handleInteraction directly from its own interactionCreate listener, last among the
// guided-setup features, preserving the first-refusal order the old emit wrappers had.
function createSetupExtensionFeature({ getUserByDiscordId, getTrustScore, audit, db, inviteUserToPlex, fetchUserQuota, log }) {
  if (!getUserByDiscordId || !getTrustScore || !audit || !db || !inviteUserToPlex || !fetchUserQuota || !log) {
    throw new TypeError('Setup extension dependencies are required');
  }
  ...
  async function handleInteraction(interaction) {
    if (!isOwnedInteraction(interaction)) return false;
    try {
  ...
  return { handleInteraction, owns: isOwnedInteraction };
}
...
function isOwnedInteraction(interaction) {
  ...
  // The Request Media wizard (src/setup-request-ui.js) is an explicit index.js interaction
  // handler, so this catch-all must not swallow its buttons. The device-state and
  // enhancements features run earlier in index.js's handler chain, so their owned ids
  // (setup:open, setup:ph_*, setup:plex_invite_username, setup:request_key:*,
  // setup:admin_tailnet_help:*) never reach this catch-all — matching the first-refusal
  // order the old emit wrappers had by install order.
