...
// Forwarded-proto behavior (#191 criterion 6): the tunnel/reverse-proxy terminates TLS and must
// forward X-Forwarded-Proto, and the app must treat such a request as secure (Secure cookies,
// req.secure). This probes the live local listener twice — with and without the header — and
// reads the differential signal off the passkey challenge cookie (dm_webauthn), which carries
// `; Secure` exactly when the app's secure-request detection fires. Credential-free and
// read-only: authentication-options only mints a client-held challenge binding; it never
// authenticates anyone and writes no server state.
async function probeForwardedProtoBehavior({ timeoutMs = 8000, port } = {}) {
  ...
  const withoutHeader = await challenge({});
  if (withoutHeader.error) {
    return check('Forwarded-proto behavior', 'warn', `control probe without the forwarded header failed: ${withoutHeader.error}`);
  }
  const secureWithout = withoutHeader.cookiePresent && withoutHeader.secure;
  if (withHeader.secure && !secureWithout) {
    return check('Forwarded-proto behavior', 'ok', 'X-Forwarded-Proto: https is honored end-to-end — the challenge cookie carries `; Secure` only when the header is present');
  }
  if (!withHeader.secure) {
    return check('Forwarded-proto behavior', 'fail', 'the app did NOT mark the challenge cookie `; Secure` despite X-Forwarded-Proto: https — the proxy may be dropping the header or the app is misreading it; session cookies will lack `; Secure` through the public origin');
  }
  return check('Forwarded-proto behavior', 'warn', 'the challenge cookie is `; Secure` even without the forwarded header — unexpected on a plain-HTTP listener; investigate before trusting the signal');
}
...
async function checkPublicOriginReadiness({ timeoutMs = 8000, publicPort = 443, config = require('./config').CONFIG } = {}) {
  ...
  checks.push(await probeForwardedProtoBehavior({ timeoutMs, port: config.PORT }));
