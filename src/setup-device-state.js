...
//
// Explicit interaction registration (no Discord prototype patching): index.js instantiates
// createSetupDeviceStateFeature with its dependencies and calls handleInteraction directly
// from its own interactionCreate listener, ahead of the enhancements and extension features,
// preserving the first-refusal order the old emit wrappers had by install order.
...
function createSetupDeviceStateFeature({ getUserByDiscordId, getSetting, setSetting, deleteSetting, audit, log }) {
  if (!getUserByDiscordId || !getSetting || !setSetting || !deleteSetting || !audit || !log) {
    throw new TypeError('Setup device state dependencies are required');
  }
  ...
  async function handleInteraction(interaction) {
    ...
    try {
      if (interaction.isChatInputCommand?.() && interaction.commandName === 'setup') {
        ...
        const id = String(interaction.customId || '');
        if (id === 'setup:open') await showSetup(interaction);
        else if (id === 'setup:ph_connection') await showPhDashboard(interaction);
        else if (id.startsWith('setup:ph_device:')) await showDevice(interaction, id.split(':')[2]);
        else if (id.startsWith('setup:ph_confirm:')) await confirmDevice(interaction, id.split(':')[2]);
        else if (id.startsWith('setup:ph_unconfirm:')) await resetDevice(interaction, id.split(':')[2]);
      }
    } catch (err) {
