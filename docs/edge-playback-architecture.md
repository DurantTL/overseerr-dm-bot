...
* ⚠️ **Real Syncthing folder IDs + library paths are committed** as doc examples
  (`agent/README.md`, `agent/agent.js`, `src/db.js`, `index.js`): `aaaaa-bbbbb`, `ccccc-ddddd`,
  `eeeee-fffff`, `ggggg-hhhhh` alongside `/mnt/media/Media/{Family Films,4k,Movies,TV Shows}`.
