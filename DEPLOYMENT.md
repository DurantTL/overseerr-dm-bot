...
Set `BOT_IMAGE_TAG=sha-<good-commit>` (the immutable per-commit tag) — or
`BOT_IMAGE_TAG=vX.Y.Z` if that commit was a tagged release (see "Versioned
releases" below) — and redeploy. Because Watchtower only tracks the tag you
reference, pinning to either an immutable `sha-` or `vX.Y.Z` tag also freezes
auto-updates until you move back to `latest`.

### Versioned releases and image security (#184)

Every image pushed to GHCR (`latest`, `sha-<commit>`, and any `vX.Y.Z`) is
scanned for known vulnerabilities before it publishes: a **fixable
CRITICAL or HIGH severity** finding fails the build workflow and nothing
reaches GHCR. Findings with no fix available yet, or below HIGH severity,
are reported in the workflow log but never block — otherwise the gate would
eventually block on a vulnerability nobody upstream has patched yet, or cry
wolf on every low-severity finding until people stop reading it. Both the
main bot image and the tier-agent image are scanned in the pull-request
gate (`.github/workflows/test.yml`); only the bot image is published, and
only it carries the SBOM/provenance attestations below.

Most CRITICAL/HIGH findings the scanner has actually caught here came not
from this project's own dependencies but from the `node:24-slim`/
`node:24-alpine` base images' bundled `npm` installation, which vendors its
own copies of packages like `tar`, `brace-expansion`, and `ip-address`.
Neither image needs `npm` at runtime — each `CMD`/`ENTRYPOINT` invokes
`node` directly — so both Dockerfiles remove `npm`, `npx`, and `corepack`
after the dependency install step, and run an OS package upgrade
(`apt-get upgrade` / `apk upgrade`) to pick up Debian/Alpine security
point-releases between base-image refreshes. If a future finding lands in
this project's own `dependencies` (not `devDependencies`, which never ship
in the image) rather than in the base image, fix it the normal way: bump
the package in `package.json`/`package-lock.json`.

Every published bot image also carries a build **SBOM** (software bill of
materials) and **provenance** attestation, generated by `docker buildx`
during the publish step. Inspect either without pulling the image:

```bash
docker buildx imagetools inspect ghcr.io/<owner>/overseerr-dm-bot:latest \
  --format '{{ json .SBOM }}'
docker buildx imagetools inspect ghcr.io/<owner>/overseerr-dm-bot:latest \
  --format '{{ json .Provenance }}'
```

**Cutting a release:** bump `"version"` in `package.json` to `X.Y.Z`, commit
and merge it to `main` like any other change, then tag that commit and push
the tag:
...
Pushing a `vX.Y.Z` tag re-runs the same test → scan → build workflow as a
normal `main` push, additionally tags the image `vX.Y.Z` (and the shorthand
`X.Y`) in GHCR, and creates a GitHub Release with auto-generated notes from
the commits since the previous tag. It does **not** touch `latest` or your
running deployment — a release is a labeled, immutable pin you can choose to
deploy (`BOT_IMAGE_TAG=vX.Y.Z`), not an automatic rollout. Not every commit
needs a release; tag the ones worth a stable reference point (a deploy
milestone, a point you might want to roll back to by name instead of by
commit SHA).

GitHub Actions used in these workflows are pinned to immutable commit SHAs
(with a `# vX` comment for readability) rather than a moving tag, so a
compromised or rewritten upstream tag can't silently change what CI runs;
Dependabot (`.github/dependabot.yml`, `github-actions` ecosystem) opens a PR
to bump the pinned commit whenever a new release comes out.
